...
<?php include 'account_inc/exp_research.php';
	compter_visite();
	$sql = "SELECT * FROM expedition WHERE deleted =0 AND featured = 1";
	$result = $db->query($sql);
?>

<div class="container">
	<div class="t_announce">
		<h2 class="text-center">Dernieres annonces d'expedition de colis</h2>
	</div>
				...
				<div class="wrap">
					...
					<div class="results">
						<?php while($presult = mysqli_fetch_assoc($result)): 
							...
							$c_leave = $presult['expedition_date'];
							$leave = date('d-m-Y', strtotime ($c_leave));
						?>
						
							<li class="searchs_results">
								<a class="oneresults" href="details_expedition.php?edit=<?=$presult['expedition_id'];?>">
									<article class="rows">
										<div class="users">
