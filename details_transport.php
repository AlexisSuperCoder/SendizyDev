...
if(isset($_GET['edit']) && !empty($_GET['edit'])){
		...
		$sql3 = "SELECT * FROM travel WHERE travel_id = '$edit_id'"; 
		$edit_result = $db->query($sql3);
		$eresult = mysqli_fetch_assoc($edit_result);
		$edate = $eresult['travel_date'];
...
<?php include 'account_inc/tra_research.php';?>
		...
		<div class="container">
			...
			<div class="exp_title">
				Voyage de <?=$eresult['departure'];?>, <?=$dep_country['country'];?> vers <?=$eresult['destination'];?>, <?=$des_country['country'];?> le <?=$day;?>
			...
			<div>
				...
				<div class="exp_voyage_it">
					Date de voyage : <?= date("d/m/Y", strtotime($eresult['travel_date']));?>.
				</div>
			...
			<div>
				...
				<div class="exp_voyage_it">
					Date limite de reception de colis : <?= date("d/m/Y", strtotime($eresult['limit_day']));?>.
				</div>
			...
			Vous souhaitez expedier un colis à destination de <?=$eresult['destination'];?>, <?=$des_country['country'];?>:<br>
					<a href="publier_expedition.php"><button type="button" class="btn btn-success btn-lg">Publier votre annonce</button></a>
